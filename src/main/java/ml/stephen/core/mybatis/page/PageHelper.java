...
public class PageHelper implements Interceptor {
	...
	private static final ThreadLocal<PageList> LOCAL_PAGE = new ThreadLocal<PageList>();
	...
	public static void startPage(int pageNum, int pageSize, boolean count) {
		LOCAL_PAGE.set(new PageList(pageNum, pageSize, count));
	}
	
	/**
	 * 移除分页查询
	 */
	public static void removePageHelper(){
		LOCAL_PAGE.remove();
	}
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (LOCAL_PAGE.get() == null && rowBounds == RowBounds.DEFAULT) {
			...
			// 分页sql - 重写sql
			msObject.setValue(BOUND_SQL, getPageSql(sql, pageList));
			// 恢复类型
	 ...
	 * @return
	 */
	private String getPageSql(String sql, PageList pageList) {
		StringBuilder pageSql = new StringBuilder(200);
		if ("mysql".equals(dialect)) {
			pageSql.append(sql);
			pageSql.append(" limit " + pageList.getStartRow() + "," + pageList.getPageSize());
		} else if ("hsqldb".equals(dialect)) {
			pageSql.append(sql);
			pageSql.append(" LIMIT " + pageList.getPageSize() + " OFFSET " + pageList.getStartRow());
		} else if ("oracle".equals(dialect)) {
			pageSql.append("select * from ( select temp.*, rownum row_id from ( ");
			pageSql.append(sql);
			pageSql.append(" ) temp where rownum <= ").append(pageList.getEndRow());
			pageSql.append(") where row_id > ").append(pageList.getStartRow());
		}
